# 셀러계정(Organization) 시스템 가이드

## 개요

회원사의 다수 담당자를 지원하는 셀러계정 시스템입니다. 신선푸드 대표와 직원들이 같은 계정(셀러계정)에서 활동하며 주문, 발주서 등의 데이터를 공유할 수 있습니다.

## 주요 기능

### 1. 셀러계정 구조
- **1개 셀러계정(회사) = 여러 멤버(직원)**
- 각 멤버는 개별 계정을 가지지만 같은 셀러계정의 데이터를 공유
- 셀러계정 소유자/관리자/일반 멤버 등 역할 구분

### 2. 데이터 공유
같은 셀러계정의 모든 멤버는 다음 데이터를 공유합니다:
...
### organization_members (셀러계정 멤버)
```sql
- id: UUID (PK)
- organization_id: 셀러계정 ID
- user_id: 사용자 ID
...
-- users 테이블
ALTER TABLE users
ADD COLUMN primary_organization_id UUID; -- 주 소속 셀러계정

-- integrated_orders 테이블
ALTER TABLE integrated_orders
ADD COLUMN organization_id UUID; -- 주문이 속한 셀러계정

-- platform_orders 테이블 (있다면)
ALTER TABLE platform_orders
ADD COLUMN organization_id UUID;

-- user_cash, user_credits 테이블 (셀러계정 단위 관리 선택적)
...
### 셀러계정 관리
```
GET    /api/organizations          # 내가 속한 셀러계정 목록
POST   /api/organizations          # 새 셀러계정 생성
PATCH  /api/organizations?id=xxx   # 셀러계정 정보 수정
```
...
POST   /api/organizations/join            # 초대 수락 및 셀러계정 가입
```
...
- RLS 정책 설정
- **기존 사용자를 위한 개인 셀러계정 자동 생성**
- 기존 주문에 셀러계정 ID 자동 매핑

### 2. 자동 셀러계정 생성 시스템

**모든 신규 회원에게 자동으로 셀러계정이 생성됩니다:**
...
기본 셀러계정 자동 생성
  ...
  ↓
마이페이지에서 회사 정보 입력 시
  - 셀러계정명 자동 변경: "[이름]의 셀러계정" → "신선푸드"
  - 사업자 정보 동기화
```

**셀러계정명 우선순위:**
1. `company_name` (회사명) - 입력 시 자동 변경
2. `profile_name`의 셀러계정 - 프로필명이 있으면
3. `name`의 셀러계정 - 이름이 있으면
4. `email`의 셀러계정 - 최후 수단
...
- 관리자: 모든 주문 조회
- 일반 사용자: 같은 셀러계정의 주문만 조회
- 셀러계정 없는 사용자: 본인 주문만 조회

### 2. 주문 등록 시 셀러계정 ID 자동 설정
다음 API에서 자동으로 셀러계정 ID가 설정됩니다:
- `POST /api/integrated-orders` (단건)
- `POST /api/integrated-orders/bulk` (대량)
- `POST /api/platform-orders` (플랫폼 주문)

### 3. 데이터 보안 (RLS)
- 셀러계정 정보는 해당 셀러계정 멤버만 조회 가능
- 멤버 정보는 같은 셀러계정 멤버만 조회 가능
- 초대 정보는 셀러계정 관리자만 조회/관리 가능

## 주의사항

### 1. 기존 사용자 마이그레이션
- 마이그레이션 실행 시 기존 partner/customer 역할 사용자에게 개인 셀러계정이 자동 생성됩니다
- 기존 주문에 자동으로 셀러계정 ID가 매핑됩니다

### 2. 캐시/크레딧 공유
- 기본적으로 캐시/크레딧은 개인 단위로 관리됩니다
- 셀러계정 단위로 관리하려면 마이그레이션 파일의 주석 처리된 부분을 해제하세요
