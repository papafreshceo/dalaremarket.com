-- ===========================
-- get_order_statistics 함수 재생성 (organizations 조인 제거)
-- ===========================
-- seller_name은 API 레벨에서 조회하도록 변경
...
'통합 주문 통계 계산 함수 - 조직 필터링 지원 (seller_name은 API에서 조회)
Returns: JSON object with status_stats, vendor_stats, seller_stats (organization_id only), option_stats';

-- 3. 성공 메시지
DO $$
BEGIN
  RAISE NOTICE '✅ get_order_statistics 함수가 성공적으로 재생성되었습니다 (SECURITY DEFINER 사용).';
END $$;
