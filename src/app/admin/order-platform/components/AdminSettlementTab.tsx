...
// platform/orders Order 타입 (정산에 필요한 필드만)
interface Order {
  id: number;
  orderNo: string;
  orderNumber?: string;
...
export default function AdminSettlementTab({ integratedOrders, organizationNames }: AdminSettlementTabProps) {
  ...
  const downloadPDF = async (monthData: any) => {
    ...
    await downloadMonthlyStatementPDF(year, month, filteredOrders, buyerInfo, sellerInfo);
  };
              ...
              <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
                ...
                <th style={{
                  ...
                  minWidth: '90px'
                      ...
                      <tbody>
                        {month.dailyData.map((row: any, index: number) => (
                <tr
                  key={index}
                  style={{
                    borderBottom: '1px solid var(--color-border)',
                ...
                <div style={{ position: 'relative' }}>
                  <button
                    onClick={async () => {
                      ...
                      await downloadPeriodSettlementExcel(startDate, endDate, filteredOrders);
                    }}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.backgroundColor = 'var(--color-surface-hover)';
                ...
                <div style={{ position: 'relative' }}>
                  <button
                    onClick={async () => {
                      ...
                      await downloadPeriodStatementPDF(startDate, endDate, filteredOrders, buyerInfo, sellerInfo);
                    }}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.backgroundColor = 'var(--color-surface-hover)';
