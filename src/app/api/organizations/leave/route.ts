...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🗑️  [탈퇴] 멤버 삭제 시작:', memberData.id)
    ...
    if (deleteError) {
      console.error('❌ [탈퇴] 멤버 삭제 실패:', deleteError)
      return NextResponse.json(
        { error: '셀러계정 탈퇴에 실패했습니다', details: deleteError.message },
        { status: 500 }
      )
    }
    console.log('✅ [탈퇴] 멤버 삭제 완료')

    // 4. primary_organization_id를 NULL로 설정 (새로운 개인 셀러계정을 생성하기 위해)
    console.log('🔄 [탈퇴] primary_organization_id를 NULL로 설정')
    ...
    if (nullifyError) {
      console.error('❌ [탈퇴] primary_organization_id NULL 설정 실패:', nullifyError)
      throw nullifyError
    }
    console.log('✅ [탈퇴] primary_organization_id NULL 설정 완료')

    // 5. 개인 셀러계정 자동 생성
    let newOrganizationId: string | null = null
    try {
      console.log('🚀 [탈퇴] 개인 셀러계정 생성 시작, user_id:', auth.user.id)
      const result = await autoCreateOrganizationFromUser(auth.user.id)
      console.log('✅ [탈퇴] 개인 셀러계정 생성 결과:', result)

      if (!result.organization_id) {
        console.error('❌ [탈퇴] organization_id가 없음:', result)
      ...
      console.log('🔄 [탈퇴] primary_organization_id 업데이트 시작:', {
      ...
      if (updateError) {
        console.error('❌ [탈퇴] primary_organization_id 업데이트 실패:', updateError)
        throw updateError
      }
      console.log('✅ [탈퇴] primary_organization_id 업데이트 성공:', updateData)

      // 7. 셀러코드 자동 생성
      console.log('🔑 [탈퇴] 셀러코드 생성 시작')
      const codes = await generateUserCodes(auth.user.id)
      console.log('✅ [탈퇴] 셀러코드 생성 완료:', codes)
      ...
      console.error('❌ [탈퇴] 개인 셀러계정 생성 실패:', createError)
    ...
    console.error('셀러계정 탈퇴 오류:', error)
    return NextResponse.json(
