...
export async function POST(request: NextRequest) {
  try {
    ...
    // 캐시 회수 함수 호출
    const { data: result, error } = await supabase.rpc('revoke_cash', {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { success: false, error: `캐시 회수에 실패했습니다: ${error.message}` },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: `${result.cash_revoked.toLocaleString()}원이 회수되었습니다.`,
      data: result
    });
