...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      logger.error('랭킹 점수 설정 조회 실패:', error);
    ...
    logger.error('GET /api/admin/ranking-score-settings 오류:', error);
    return NextResponse.json(
      { success: false, error: error.message },
      { status: 500 }
    );
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (error) {
      logger.error('랭킹 점수 설정 업데이트 실패:', error);
      return NextResponse.json(
        { success: false, error: error.message },
        { status: 500 }
      );
    ...
    logger.error('PUT /api/admin/ranking-score-settings 오류:', error);
    return NextResponse.json(
      { success: false, error: error.message },
      { status: 500 }
    );
