...
export default function WinWinPage() {
  const [activeService, setActiveService] = useState<'photo' | 'farm'>('photo');
  ...
  const [editQuantities, setEditQuantities] = useState<Quantities>({});
  ...
  // 체크박스 토글
  const toggleLocationCheck = (location: string, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  // 편집 수량 조정
  const adjustEditQuantity = (id: string, change: number) => {
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <input
                        ...
                        onChange={(e) => toggleLocationCheck(key, e as any)}
