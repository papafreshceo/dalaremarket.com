...
export default function Footer() {
  const footerLinks = [
    { path: '/platform', text: '대시보드' },
    { path: '/platform/products/all', text: '상품리스트' },
      ...
      <div style={{
        ...
        <div style={{
          ...
          {footerLinks.map((item, index) => (
            <span key={item.path} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
              {index > 0 && <span>|</span>}
              <Link
                href={item.path}
                style={{
        ...
        {/* 2행: 회사정보 + 연락처 + SNS + 카피라이트 */}
        <div style={{ fontSize: '9px', color: '#6b7280', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', flexWrap: 'wrap' }}>
          {/* 회사정보 */}
          <span>대표: 남잠화</span>
          <span>|</span>
          <span>사업자: 107-30-96371</span>
          <span>|</span>
          <span>통신판매: 2022-경북청도-0003</span>
          <span>|</span>
          ...
          <div style={{ display: 'inline-flex', gap: '4px' }}>
            {socialLinks.map((social, index) => (
              ...
              >
                <svg style={{ width: '9px', height: '9px', fill: '#6b7280' }} viewBox="0 0 24 24">
                  <path d={social.icon} />
                </svg>
              </a>
            ))}
          ...
          {/* 카피라이트 + 약관 */}
          <span style={{ color: '#9ca3af' }}>© 2024 달래마켓. All rights reserved.</span>
          <span>|</span>
